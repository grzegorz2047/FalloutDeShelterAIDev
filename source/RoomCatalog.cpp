...
RoomDefinition room(const char* id, const char* category, const char* special,
                    int cost, int width, int population, int progress,
                    const char* achievement, const char* produces, int storage) {
...
bool RoomCatalog::valid(std::string* detail) const {
    ...
    static const std::unordered_set<std::string> categories = {
        ...
        "recruitment", "crafting", "cosmetic", "special"};
    static const std::string special = "SPECIAL";

    for (const auto& definition : definitions_) {
        if (definition.id.rfind("room.", 0) != 0 || !ids.insert(definition.id).second) {
        ...
        if (definition.name_key.empty() || definition.description_key.empty() || definition.icon.empty()) {
            if (detail) *detail = "room presentation metadata is incomplete: " + definition.id;
            return false;
        }
        if (special.find(definition.special) == std::string::npos || definition.base_cost <= 0 ||
            definition.width < 1 || definition.width > 3 || definition.max_level < 1 ||
...
const RoomDefinition* RoomCatalog::find(const std::string& id) const noexcept {
    const auto it = std::find_if(definitions_.begin(), definitions_.end(),
                                 [&](const RoomDefinition& value) { return value.id == id; });
...
RoomAvailability RoomCatalog::availability(const std::string& id,
    ...
    if (context.progress < definition->unlock_progress) {
        ...
        return {definition, false, reason.str(), "Complete objectives to advance shelter progress."};
    }
...
std::vector<RoomAvailability> RoomCatalog::list(const UnlockContext& context) const {
    ...
    for (const auto& definition : definitions_) values.push_back(availability(definition.id, context));
    return values;
}

const RoomDefinition& RoomCatalog::resolve_or_placeholder(const std::string& id) const noexcept {
    const RoomDefinition* definition = find(id);
...
std::vector<RoomDefinition> built_in_room_definitions() {
    return {
        room("room.power_generator", "production", "S", 100, 1, 0, 0, "", "resource.power", 0),
        room("room.water_purifier", "production", "P", 120, 1, 4, 0, "", "resource.water", 0),
        room("room.kitchen", "production", "A", 120, 1, 4, 0, "", "resource.food", 0),
        room("room.reactor", "production", "I", 1600, 2, 40, 4, "", "resource.power", 0),
        room("room.hydroponics", "production", "A", 1450, 2, 36, 4, "", "resource.food", 0),
        room("room.filtration", "production", "P", 1450, 2, 36, 4, "", "resource.water", 0),
        room("room.power_storage", "storage", "E", 300, 1, 12, 1, "", "", 250),
        room("room.food_storage", "storage", "E", 300, 1, 12, 1, "", "", 250),
        room("room.water_storage", "storage", "E", 300, 1, 12, 1, "", "", 250),
        room("room.living_quarters", "residential", "C", 150, 1, 0, 0, "", "", 2),
        room("room.strength_gym", "training", "S", 600, 1, 20, 2, "", "", 0),
        room("room.perception_lab", "training", "P", 650, 1, 22, 2, "", "", 0),
        room("room.agility_course", "training", "A", 700, 1, 24, 2, "", "", 0),
        room("room.medbay", "medical", "I", 500, 1, 16, 1, "", "resource.medkits", 20),
        room("room.decontamination", "medical", "I", 800, 1, 28, 3, "", "resource.antirad", 20),
        room("room.radio_station", "recruitment", "C", 900, 2, 26, 2, "", "", 0),
        room("room.workshop", "crafting", "I", 1100, 2, 30, 3, "", "", 0),
        room("room.outfit_bench", "crafting", "I", 1200, 2, 32, 3, "", "", 0),
        room("room.community_hall", "cosmetic", "C", 750, 2, 18, 2, "", "", 0),
        room("room.command_center", "special", "L", 2500, 3, 50, 5,
             "achievement.first_expedition", "", 0),
    };
}
