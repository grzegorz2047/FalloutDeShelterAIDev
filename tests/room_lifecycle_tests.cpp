...
int main() {
    ...
    for (const auto& segment : lifecycle.normalized_segments()) assert(segment.group_id == 1);
    ...
    assert(lifecycle.confirm_upgrade(1, 300, 1001));
    assert(lifecycle.credits() == 700);
    assert(!lifecycle.confirm_upgrade(1, 300, 1001));
    assert(lifecycle.credits() == 700);

    const std::uint64_t upgraded_group = lifecycle.segments().front().group_id;
    assert(lifecycle.confirm_upgrade(upgraded_group, 200, 1002));
    ...
    assert(!lifecycle.confirm_demolish(2, 40, 10, 2001));
    assert(lifecycle.credits() == 500);
    assert(lifecycle.confirm_demolish(2, 40, 11, 2001));
    assert(lifecycle.credits() == 540);
    assert(!lifecycle.confirm_demolish(2, 40, 100, 2001));
    assert(lifecycle.credits() == 540);
    ...
    assert(!poor.confirm_upgrade(20, 50, 3001));
